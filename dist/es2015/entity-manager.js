...
export let EntityManager = class EntityManager {
  ...
  find(Entity, id) {
    return Promise.resolve().then(() => {
      ...
      return cache.get(uri) || serverMap.get(this).get(uri).then(data => this.create(Entity, data)).then(entity => cachedEntity(entity, cache, uri));
    });
  }
  ...
  query(Entity, stringOrPropertyMap = {}) {
    return Promise.resolve().then(() => {
      ...
      return serverMap.get(this).get(path, stringOrPropertyMap).then(entityMapper).then(map => {
        ...
        return Promise.all(entries.map(entry => this.create(entry[1], entry[0])));
