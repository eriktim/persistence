  ...
  persist(entity) {
    return Promise.resolve().then(() => {
      ...
      if (!id || PersistentData.isDirty(entity)) {
        ...
        return Promise.resolve().then(() => applySafe(config.prePersist, entity)).then(() => Reflect.apply(fetch, serverMap.get(this), [id ? `${ path }/${ id }` : path, data])).then(raw => raw && PersistentObject.setData(entity, raw)).then(() => attach(this, entity)).then(() => applySafe(config.postPersist, entity));
      }
    }).then(() => entity);
  }
