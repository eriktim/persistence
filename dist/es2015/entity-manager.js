...
function toParams(...maps) {
  ...
  let params = Object.keys(flatMap).map(key => encodeURIComponent(key) + '=' + encodeURIComponent(flatMap[key])).join('&');
...
export let EntityManager = class EntityManager {
  ...
  find(Entity, id) {
    return Promise.resolve().then(() => {
      ...
      let noCache = () => {
        ...
        return serverMap.get(this).get(uri).then(data => this.create(Entity, data)).then(entity => cachedEntity(entity, cache, uri));
      };
  ...
  query(Entity, stringOrPropertyMap = {}) {
    return Promise.resolve().then(() => {
      ...
      let path = getPath(Entity);
      let cache = cacheMap.get(this);
      return serverMap.get(this).get(path, stringOrPropertyMap).then(entityMapper).then(batchesOrMap => {
        let batches = batchesOrMap instanceof Map ? [batchesOrMap] : Array.from(batchesOrMap);
        batches.forEach(map => {
        ...
        return batches.reduce((promise, map) => {
          ...
          return promise.then(entities => {
            return Promise.all(entries.map(entry => {
              return this.create(entry[1], entry[0]).then(entity => {
                if (!hasId(entity)) {
                  return entity;
                }
  ...
  persist(entity) {
    return Promise.resolve().then(() => {
      ...
      return Promise.all(Array.from(entity[RELATIONS]).map(e => this.persist(e)));
    }).then(() => {
      if (unresolvedRelationsMap.has(entity)) {
      ...
      if (noId || PersistentData.isDirty(entity)) {
        ...
        return Promise.resolve().then(() => applySafe(config.prePersist, entity)).then(() => Reflect.apply(fetch, server, [noId ? path : `${ path }/${ id }`, data])).then(raw => {
          if (noId) {
            let location = raw[LOCATION];
            if (!location) {
              throw new Error('REST server should return' + ' the location of the new entity');
            }
            let idPath = location.substring(location.lastIndexOf(path) + path.length + 1);
  ...
  refresh(entity, reload = false) {
    return Promise.resolve().then(() => {
      assertEntity(this, entity);
      let id = getId(entity);
      return this.find(Util.getClass(entity), id).then(newEntity => {
  ...
  remove(entity) {
    return Promise.resolve().then(() => {
      ...
      let path = getPath(entity);
      let config = PersistentConfig.get(entity);
      return Promise.resolve().then(() => applySafe(config.preRemove, entity)).then(() => id ? serverMap.get(this).delete(`${ path }/${ id }`) : undefined).then(() => entity[REMOVED] = true).then(() => applySafe(config.postRemove, entity)).then(() => this.detach(entity)).then(() => entity);
    });
  }
};
